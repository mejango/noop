...
function usage() {
  return [
    ...
    '  --buyback-max-edge=30 --buyback-min-capture-pct=0.40 --buyback-min-ask=1.5 --taker-fee-per-contract=1.25',
    '',
    'Learning:',
...
function main() {
  ...
  try {
    ...
    const simulationConfig = {
      ...
      buybackMaxEdge: parseNumber(args['buyback-max-edge'], null),
      buybackMinCapturePct: parseNumber(args['buyback-min-capture-pct'], 0.40),
      buybackMinAsk: parseNumber(args['buyback-min-ask'], 1.5),
      takerFeePerContract: parseNumber(args['taker-fee-per-contract'], 0),
      greedEdge: parseNumber(args['greed-edge'], null),
      breakoutPct: parseNumber(args['breakout-pct'], null),
      breakoutLookbackDays: parseNumber(args['breakout-lookback-days'], 7),
      breakoutExcludeHours: parseNumber(args['breakout-exclude-hours'], 24),
      greedExposureCap: parseNumber(args['greed-exposure-cap'], 0.65),
