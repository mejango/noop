  ...
  for (const snapshot of positionSnapshots || []) {
    if (snapshot?.direction === 'long' && snapshot?.option_type === 'P') {
      requirements.push({
        ...
        instruction: `Create a reduce-only sell_put watcher only for roll/expiry management at <=${PUT_ROLL_DTE_THRESHOLD} DTE or asymmetric monetization after executable PnL reaches >=${PUT_MONETIZATION_PROFIT_THRESHOLD}%.`,
      });
    }
...
const getSellPutProtectionGate = (rule, values) => {
  ...
  return {
    allowed: false,
    reason: `protective long put exit requires dte <= ${PUT_ROLL_DTE_THRESHOLD} or executable pnl >= ${PUT_MONETIZATION_PROFIT_THRESHOLD}%; actual dte=${dteText}, executable_pnl=${pnlText}`,
  };
};
...
const getStandingRulebookDisciplinePrompt = () => [
  ...
  `- sell_put rules define when an owned long put should be rolled near expiry or monetized after true asymmetric upside. They must not sell long-dated protection merely because it has recoverable bid value; before ${PUT_ROLL_DTE_THRESHOLD} DTE, require executable unrealized_pnl_pct >= ${PUT_MONETIZATION_PROFIT_THRESHOLD}%.`,
...
- For sell_put exits: use conditions on dte (e.g. dte lte ${PUT_ROLL_DTE_THRESHOLD}) or executable unrealized_pnl_pct. IMPORTANT: sell_put means selling an already-owned long put to close or roll it. It is reduce_only and must never be interpreted as opening a naked short put. Do not sell a long-dated protective put merely because it has recoverable bid/mark value. Before ${PUT_ROLL_DTE_THRESHOLD} DTE, a monetization watcher must require executable unrealized_pnl_pct gte ${PUT_MONETIZATION_PROFIT_THRESHOLD}. Selling an owned long put is capital-releasing: it returns cash/premium recovery, reduces the hedge position, and does NOT consume more margin. It will generally improve headroom, not worsen it. If you avoid triggering a sell_put, do it because removing protection is strategically unwise or value has not delivered asymmetric upside, not because the exit itself uses more margin.
...
- Roll (sell_put exit) at ~${PUT_ROLL_DTE_THRESHOLD} DTE to avoid terminal theta decay while preserving convexity. Before that, only monetize protective puts after executable unrealized_pnl_pct reaches at least ${PUT_MONETIZATION_PROFIT_THRESHOLD}%.
