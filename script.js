...
CALL DISCIPLINE: Short calls have a hard cap at ${(CALL_EXPOSURE_CAP_PCT * 100).toFixed(0)}% inferred Derive margin utilization, but new entries should stay below the ${(CALL_ENTRY_CAP_PCT * 100).toFixed(0)}% entry buffer. Reject call sells that would lean on the full hard cap or are too small to matter.
...
PUT EXIT DISCIPLINE: For sell_put — evaluate it as monetizing or rolling an existing long put. Do not analyze it as short put selling or naked downside exposure. Selling an owned long put is capital-releasing: it returns cash/premium recovery and reduces the hedge position. If you reject a sell_put, do it because removing protection is strategically unwise, not because the exit itself consumes more margin. The question is whether closing this owned hedge now is prudent, not whether opening short put risk is acceptable.
...
PUT EXIT DISCIPLINE: For sell_put, judge whether monetizing or rolling an owned long hedge is sensible. Never treat sell_put as opening naked short put exposure. Selling an owned long put is capital-releasing: it returns cash/premium recovery and reduces the hedge position. If you reject a sell_put, do it because removing protection is strategically unwise, not because the exit itself consumes more margin.
