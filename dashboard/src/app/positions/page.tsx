  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h1 className="text-xl font-bold text-juice-orange">Positions</h1>
            ...
            return (
              <div key={pos.id}>
                <button
                  ...
                  className="w-full text-left px-4 py-3 rounded-lg glass glass-hover"
                >
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      <Badge label={pos.status} color={statusColors[pos.status] || 'default'} />
                      <Badge label={pos.direction === 'buy' ? 'PUT' : 'CALL'} color={pos.direction === 'buy' ? 'red' : 'green'} />
                      <span className="text-sm">{pos.instrument_name}</span>
                    </div>
                ...
                {isExpanded && (
                  <div className="ml-8 mt-1 mb-2 space-y-1">
                    {trades.length === 0 ? (
                      <p className="text-gray-500 text-xs py-2">No trades for this position</p>
                    ) : trades.map(t => (
                      <div key={t.id} className="flex items-center gap-3 text-xs px-3 py-1.5 rounded bg-white/5 border border-white/10">
                        <span className="text-gray-500 w-24">{new Date(t.timestamp).toLocaleDateString()}</span>
                        ...
                        {t.fee && <span className="text-gray-500">Fee: {formatUSD(t.fee)}</span>}
