          ...
          <Card>
            ...
            <ResponsiveContainer width="100%" height={200}>
              <ComposedChart data={chartData} margin={margins} barGap={0} barCategoryGap={0}>
                ...
                {hasVolume && volDexes.map((name, i) => (
                  <Bar key={`${name}_vol`} yAxisId="vol" dataKey={`${name}_volNorm`} fill={getColor(name, i)} fillOpacity={0.35} stroke={getColor(name, i)} strokeOpacity={0.5} isAnimationActive={false} />
                ))}
              </ComposedChart>
            </ResponsiveContainer>
            </div>
