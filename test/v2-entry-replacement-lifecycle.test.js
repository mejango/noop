...
test('put replacement applies the composite edge threshold to fresh prices and the desired entry price', (t) => {
  const state = economicsFixture(t);
  state.activeRule.criteria.min_edge_score = 80;
  state.selectionScore = 79;
  const tickerMap = { [putName]: { a: 99, b: 98 }, [callName]: { b: 15, a: 16 } };
  const rejected = state.economics({ tickerMap });
  assert.equal(rejected.allowed, false);
  assert.match(rejected.reason, /composite put edge/i);
  ...
  assert.equal(accepted.research.selection_score, 80);
});
...
test('actual final callback rechecks replacement research and blocks a maker retry after edge deteriorates', async (t) => {
  const state = economicsFixture(t);
  state.activeRule.criteria.min_edge_score = 80;
  state.db.updateRestingOrder('incumbent', 'cancelled', 1, 8);
  let tickerReads = 0, marginReads = 0, positionReads = 0, bookReads = 0;
  const finalPolicyCalls = [];
  const bindings = {
    ...state.economicsBindings,
    db: { ...state.db, getActiveRules: () => [state.activeRule] },
    validateEntryReplacementEconomics: state.validateEconomics,
    fetchFreshTickerForInstrument: async name => { assert.equal(name, putName); tickerReads++; return state.ticker; },
    fetchSubaccount: async () => { marginReads++; return state.margin; },
    fetchPositions: async options => { assert.equal(options.throwOnError, true); positionReads++; return state.positions; },
    fetchOpenOrders: async options => { assert.equal(options.throwOnError, true); bookReads++; return []; },
    getCallMarginDecision: () => ({ available: true, entryCapSatisfied: true }),
    getFinalOrderPolicy: () => ({}),
    validateFinalOrderPolicy: input => { finalPolicyCalls.push(input); return { allowed: true }; },
  };
  ...
  const validate = createValidator({ action: state.action, instruments: state.instruments, spotPrice: 2500,
  ...
  const initial = await validate({ price: 8, amount: 4 });
  assert.equal(initial.allowed, true, initial.reason);
  state.selectionScore = 79;
  state.ticker = { ...state.ticker, a: 20, b: 6, I: 2600 };
  const retry = await validate({ price: 7.9, amount: 4 });
  assert.equal(retry.allowed, false);
  assert.equal(retry.code, 'put_edge_failed');
  assert.match(retry.reason, /composite put edge/i);
  assert.equal(finalPolicyCalls.length, 2, 'base final policy passed for both attempts');
  assert.equal(tickerReads, 2);
  assert.equal(marginReads, 2);
  assert.equal(positionReads, 2);
  assert.equal(bookReads, 1, 'failed research blocks before the second book guard or submission');
  assert.deepEqual(state.researchCalls.map(input => [input.entryPrice, input.askPrice, input.spotPrice]), [[8, 10, 2500], [7.9, 20, 2600]]);
});
